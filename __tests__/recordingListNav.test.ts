...
describe('빈 목록 상태 메시지 (EMPTY_MESSAGES)', () => {
  ...
  it('filter=processing → "처리 중인 파일이 없습니다"', () => {
    expect(EMPTY_MESSAGES['processing']).toBe('처리 중인 파일이 없습니다');
  });
  ...
  it('filter=starred → "즐겨찾기한 항목이 없습니다"', () => {
    expect(EMPTY_MESSAGES['starred']).toBe('즐겨찾기한 항목이 없습니다');
  });
});
